...
def TEST4(l, test_input, starting_point, relative_base, input_str_id):
    i=starting_point
    result = []
    relative_base = relative_base
    while (get_digit(l[i],0)+get_digit(l[i],1)*10)!=99:
        ...
        if opcode == 1:
            ...
            l = l_update(l, i+1, relative_base, mode1, ord(test_input[input_str_id]))
            i = i+2
            input_str_id = input_str_id+1
        elif opcode == 4:
            #print(n1, i)
            result.append(n1)
            i = i+2
            if len(result)==1:
                return result, l, i, relative_base, input_str_id
        elif opcode == 5:
            if n1 != 0:
                i = n2
            else:
